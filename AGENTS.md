...
## Invariant: test log data must come from real Pi RPC captures

**Rule:** All `testLogEntry` data used by the integration test **must** originate from a real Pi RPC wire capture, redacted via `test/integration/pi_rpc_capture_redact.py`. Synthetic or hand-crafted log entries are **not permitted** — they diverge from the actual wire format and mask rendering bugs.

**Why:** The UI parses Pi RPC events (`tool_execution_start`, `tool_execution_end`, `message_update`, etc.) and converts them to log lines. If the test data doesn't match the real wire format, the test will pass but the UI will break against live Pi output.

### Generating a capture

1. Edit `test/integration/pi_rpc_capture_redact.py`:
   - Set `PROMPT` to the prompt you want to capture
   - Set `CAPTURE_FILE` and `REDACTED_FILE` to paths that won't collide with other captures
2. Run the script:
   ```bash
   cd test/integration && python3 pi_rpc_capture_redact.py
   ```
   This runs `pi --mode rpc --no-session`, sends the prompt, captures all wire traffic, then redacts sensitive values (timestamps, hostnames, model names, PIDs, provider IDs).
3. Move the redacted capture into the repo:
   ```bash
   mv /tmp/pi-rpc-capture-redacted.log test/integration/pi_rpc_capture_<purpose>_redacted.log
   ```
   Name the file by purpose (e.g. `pi_rpc_capture_completed_redacted.log`, `pi_rpc_capture_failed_redacted.log`).
4. Add a corresponding `testLogEntries` variant (or parameterise the existing one) that calls `parsePiRPCCapture()` on the new file.
5. Wire the new entries into the test via `guest.log` RPC — **every** task tested must have its log entries delivered through the RPC channel, not injected directly into the queue.

### Failing task captures

A failed-task capture is generated by running Pi with a prompt that is expected to fail (e.g. a command that errors), or by interrupting the run partway through. The capture should include the events up to the point of failure. If Pi does not produce a clean failure, capture a partial run and append a synthetic `[TOOL_END]` error line — but mark it clearly as such in the file header.

### Checklist when adding test tasks

- [ ] The task has a corresponding `*_redacted.log` capture file in `test/integration/`
- [ ] Log entries are parsed via `parsePiRPCCapture()` (not hand-crafted)
- [ ] Entries are sent via `guest.log` RPC (not injected directly)
- [ ] The capture file header documents the prompt and any synthetic additions
